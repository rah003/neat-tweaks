...
import com.neatresults.mgnltweaks.ui.action.ShowUsagesAction.Definition;
import com.neatresults.mgnltweaks.ui.contentapp.browser.QueryableBrowserLocation;

/**
 * Action to list references to selected node.
 */
public class ShowUsagesAction extends AbstractAction<Definition> {

    private static final Logger log = LoggerFactory.getLogger(ShowUsagesAction.class);
    private EventBus eventBus;

    private JcrItemAdapter item;

    @Inject
    public ShowUsagesAction(Definition definition, JcrItemAdapter item, @Named(AdmincentralEventBus.NAME) EventBus eventBus) {
        super(definition);
    ...
    public static class Definition extends ConfiguredActionDefinition {
        ...
        public Definition() {
            setImplementationClass(ShowUsagesAction.class);
        }
    }
}
